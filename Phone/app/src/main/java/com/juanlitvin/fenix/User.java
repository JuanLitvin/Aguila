...
public class User {
    ...
    private static List<Device> jsonArrayToDeviceList(JSONArray devices) {
        ...
        for (int i = 0; i < devices.length(); i++) {
            try {
                listDevices.add(new Device(devices.getJSONObject(i).getString("code"), devices.getJSONObject(i).getString("id"), devices.getJSONObject(i).getString("id-owner"), devices.getJSONObject(i).getString("id-logged"),devices.getJSONObject(i).getString("name"), devices.getJSONObject(i).getString("owner"), devices.getJSONObject(i).getString("logged"), devices.getJSONObject(i).getString("date"), devices.getJSONObject(i).getInt("is-owner") == 1));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
